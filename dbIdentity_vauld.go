...
func (c *IdbusApiClient) CreateDbIdentitySource(ida string, intDbVault api.DbIdentityVaultDTO) (api.DbIdentityVaultDTO, error) {
	var result api.DbIdentityVaultDTO
	l := c.Logger()

	l.Debugf("CreateDbIdentitySource : %s [%s]", *intDbVault.Name, ida)
...
func (c *IdbusApiClient) UpdateDbIdentityVaultDto(ida string, intDbVault api.DbIdentityVaultDTO) (api.DbIdentityVaultDTO, error) {
	...
	req = req.StoreIdVaultReq(api.StoreIdVaultReq{IdaName: &ida, IdVault: &api.EmbeddedIdentityVaultDTO{}})
...
func (c *IdbusApiClient) GetDbIdentityVaultDto(ida string, intDbVault string) (api.DbIdentityVaultDTO, error) {
	...
	ctx := context.WithValue(context.Background(), api.ContextAccessToken, sc.Authn.AccessToken)
	req := c.apiClient.DefaultApi.GetDbIdVaul(ctx)
...
func initDbIdentityVaultDto(DbIdentityVaultDto *api.DbIdentityVaultDTO) {
	...
	DbIdentityVaultDto.AdditionalProperties["@c"] = ".InteralSaml2ServiceProviderDTO"
}
