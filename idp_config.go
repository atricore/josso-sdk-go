package cli

import (
	"errors"
	"fmt"

	api "github.com/atricore/josso-api-go"
)

/*

This builds a specific type for the IdP configuration.

Since the Java model uses polymorphism, we need to transform the abstarct type api.ProviderConfigDTO into api.SamlR2IDPConfigDTO

SAMPLE DUMP OF PROVIDERDTO TYPE

{
	Description:(*string)(nil),
	DisplayName:(*string)(nil),
	ElementId:(*string)(0xc0001a9820),
	Id:(*int64)(0xc0001abb00),
	Name:(*string)(nil),
	AdditionalProperties:map[string]interface {}
	{
		"@c": ".SamlR2IDPConfigDTO",
		"@id": 4,
		"encrypter": 5,
		"signer":map[string]interface {}{
			"@id": 5,
			"certificateAlias": "jcog-saml",
			"displayName":interface {}(nil),
			"elementId": "id8A9B3A5314E128",
			"id": 0, : IGNORED!!
			"keystorePassOnly": false,
			"name": "keystore-547427557",
			"password": "@WSX3edc",
			"privateKeyName": "jcog-saml",
			"privateKeyPassword": "@WSX3edc",
			"store":map[string]interface {}{
				"@id": 6,
				"displayName": "keystore-547427557.jks",
				"elementId":interface {}(nil),
				"id": 0,
				"name": "keystore-547427557",
				"uri": "keystore-547427557.jks",
				"value": ""
			},
		}
	}
}
*/
func FromIdPConfig(config *api.SamlR2IDPConfigDTO) (api.ProviderConfigDTO, error) {
	var result api.ProviderConfigDTO
	result.AdditionalProperties = make(map[string]interface{})
	// Build specific type
	result.AdditionalProperties["@id"] = config.AdditionalProperties["@id"]
	result.AdditionalProperties["@c"] = ".SamlR2IDPConfigDTO"

	result.Description = config.Description
	result.DisplayName = config.DisplayName
	result.ElementId = config.ElementId
	result.Name = config.Name
	result.AdditionalProperties["useSampleStore"] = config.UseSampleStore
	result.AdditionalProperties["useSystemStore"] = config.UseSystemStore

	if !*config.UseSampleStore && !*config.UseSystemStore {
		store := config.Signer
		storeProps := make(map[string]interface{})

		result.AdditionalProperties["encrypter"] = store.AdditionalProperties["@id"]
		result.AdditionalProperties["signer"] = storeProps

		storeProps["certificateAlias"] = &store.CertificateAlias
		storeProps["displayName"] = &store.DisplayName
		storeProps["elementId"] = &store.ElementId
		storeProps["KeystorePassOnly"] = &store.KeystorePassOnly
		storeProps["name"] = &store.Name
		storeProps["password"] = &store.Password
		storeProps["privateKeyName"] = &store.PrivateKeyName
		storeProps["privateKeyPassword"] = &store.PrivateKeyPassword

		resourceProps := make(map[string]interface{})
		storeProps["store"] = resourceProps

		resourceProps["@id"] = store.Store.AdditionalProperties["@id"]
		resourceProps["displayName"] = &store.Store.DisplayName
		resourceProps["elementId"] = &store.Store.ElementId
		resourceProps["name"] = &store.Store.Name
		resourceProps["uri"] = &store.Store.Uri
		//resourceProps["value"] = strings.Join(*store.Store.Value, "")
		resourceProps["value"] = &store.Store.Value

	}

	return result, nil
}

func ToIdPConfig(config *api.ProviderConfigDTO) (api.SamlR2IDPConfigDTO, error) {
	var result api.SamlR2IDPConfigDTO

	result.AdditionalProperties = make(map[string]interface{})

	// @id and @c properties
	class := config.AdditionalProperties["@c"]
	if class == nil {
		return result, errors.New("class property not found (@c)")
	}

	if class != ".SamlR2IDPConfigDTO" {
		return result, fmt.Errorf("invalid class %s", class)
	}

	// Build specific type
	result.AdditionalProperties["@id"] = config.AdditionalProperties["@id"]
	result.AdditionalProperties["@c"] = class

	result.Description = config.Description
	result.DisplayName = config.DisplayName
	result.ElementId = config.ElementId
	result.Name = config.Name
	result.UseSampleStore = api.PtrBool(config.AdditionalProperties["useSampleStore"].(bool))
	result.UseSystemStore = api.PtrBool(config.AdditionalProperties["useSystemStore"].(bool))

	if !*result.UseSampleStore && !*result.UseSystemStore {
		// Get signer/encrypter
		var storeProps map[string]interface{}
		var storeId float64
		var ok bool

		store := api.NewKeystoreDTO()
		store.AdditionalProperties = make(map[string]interface{})

		i := config.AdditionalProperties["encrypter"].(float64)
		fmt.Printf("POINTER TO ?! %v", i)

		if storeId, ok = config.AdditionalProperties["encrypter"].(float64); ok {
			storeProps = config.AdditionalProperties["signer"].(map[string]interface{})
		} else if storeId, ok = config.AdditionalProperties["signer"].(float64); ok {
			storeProps = config.AdditionalProperties["encrypter"].(map[string]interface{})
		} else {
			return result, fmt.Errorf("config does not have encrypter/signer ?")
		}

		if storeProps["@id"].(float64) != storeId {
			return result, fmt.Errorf("inconsistent config Ids %f, %f", storeId, storeProps["@id"].(float64))
		}

		store.AdditionalProperties["@id"] = storeId
		store.CertificateAlias = PtrString(storeProps["certificateAlias"])
		store.DisplayName = PtrString(storeProps["displayName"])
		store.ElementId = PtrString(storeProps["elementId"])
		store.KeystorePassOnly = PtrBool(storeProps["KeystorePassOnly"])
		store.Name = PtrString(storeProps["name"])
		store.Password = PtrString(storeProps["password"])
		store.PrivateKeyName = PtrString(storeProps["privateKeyName"])
		store.PrivateKeyPassword = PtrString(storeProps["privateKeyPassword"])

		resourceProps := storeProps["store"].(map[string]interface{})
		store.Store = api.NewResourceDTO()
		store.Store.AdditionalProperties = map[string]interface{}{}
		store.Store.AdditionalProperties["@id"] = resourceProps["@id"]
		store.Store.DisplayName = PtrString(resourceProps["displayName"])
		store.Store.ElementId = PtrString(resourceProps["elementId"])
		store.Store.Name = PtrString(resourceProps["name"])
		store.Store.Uri = PtrString(resourceProps["uri"])

		// TODO : Check for nil
		//store.Store.Value = &[]string{resourceProps["value"].(string)}
		store.Store.Value = PtrString(resourceProps["value"])

		result.Signer = store
		result.Encrypter = store

	}

	return result, nil
}
