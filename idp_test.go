...
func createTestIdentityProviderDTO(name string) *api.IdentityProviderDTO {
	...
	var auc1 []api.AuthenticationMechanismDTO
	var dauh api.DelegatedAuthenticationDTO
	...
	// ¿¿¿¿?????orig.SetIdentityLookups()
	// ¿¿¿¿?????orig.SetIgnoreRequestedNameIDPolicy()
	...
	orig.SetSubjectAuthnPolicies(saut)
	orig.SetSsoSessionTimeout(1)
	orig.SetSubjectAuthnPolicies(saut)
	orig.SetSubjectNameIDPolicy(snip)
	//¿¿¿¿?????orig.SetWantAssertionSigned()
